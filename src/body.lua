--[A] 2019.01.01 去除Vector依赖
--[B] 2019.04.01 优化shape对象的移动
...
local Circle = class("cherry_pong_Circle", BaseShape)
...
function Circle:clone()
	return Circle(self.filter, self.cx, self.cy, self.r)
end
...
local Capsule = class("cherry_pong_Capsule")
function Capsule:initialize(width, height, x, y)

	self.height = height or 10
	self.width = width or 50

	self.point1 = {x - self.width / 2 - self.height, y}
	self.point2 = {x + self.width / 2 + self.height, y}
...
function Capsule:draw()
	...
	love.graphics.circle("fill", self.point1[1], self.point1[2], self.height)
	love.graphics.circle("fill", self.point2[1], self.point2[2], self.height)
